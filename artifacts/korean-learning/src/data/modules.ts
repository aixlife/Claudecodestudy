...
export const commonModules: Module[] = [
  {
    ...
    description: '클로드 코드 앱을 설치하고, 프로젝트 폴더를 만들고, 터미널에서 첫 실행까지. 여기서부터 모든 것이 시작됩니다.',
    difficulty: 'basic',
    time: '약 5분',
    ...
    installGuide: {
      ...
      steps: [
        'claude.ai/download 방문 → 앱 다운로드',
        'Mac / Windows 선택 후 설치',
        '앱 실행 후 Anthropic 계정 로그인',
        '바탕화면이나 원하는 곳에 프로젝트 폴더 하나 만들기 (예: my-first-project)',
        '터미널을 열고 프로젝트 폴더로 이동',
      ],
    },
    thinkFirst: {
      ...
      correctIndex: 0,
      explanation: '맞아요. 클로드 코드는 내 컴퓨터에서, 내 폴더에서 직접 실행하는 도구예요.',
    },
    codeBlocks: [
      {
        label: '1단계 — 폴더 만들고 이동',
        code: `mkdir ~/my-first-project\ncd ~/my-first-project`,
        tip: '폴더 이름은 원하는 대로 바꿔도 돼요. 영어로 짓는 게 편해요.',
      },
      {
        label: '2단계 — 클로드 코드 실행',
        code: 'claude',
        tip: "처음 실행하면 로그인 화면이 나와요. Anthropic 계정으로 로그인하세요.",
      },
      {
        label: '3단계 — 첫 대화',
        code: '안녕! 이 폴더에 간단한 index.html 파일을 만들어줘.',
        tip: '클로드가 파일을 만들면 폴더에서 직접 확인해보세요.',
      },
    ],
    keywords: [
      ...
      { term: '프로젝트 폴더', explanation: '내가 만들 프로그램의 모든 파일이 들어갈 집이에요. 가게로 치면 매장 공간이죠. 여기서 클로드를 실행해야 해요.' },
    ],
    gate: {
      type: 'execute',
    ...
    ahaMessage: '축하해요! 이제 내 컴퓨터에서 AI와 함께 코딩할 준비가 됐어요!',
  },
  {
    id: 'C1',
